# Spécification : Webhook de Paiement
...
- `app/api/webhook/route.ts` - Handler webhook principal
- `types/webhook.ts` - Types TypeScript pour événements
...
5. Vercel → Environment Variables → `STRIPE_WEBHOOK_SECRET`
...
- **2026-01-27** : Création initiale (change: `add-stripe-webhook`)
  - Implémentation complète du webhook
  - 7 requirements validés
  - Sécurité et idempotence garanties
